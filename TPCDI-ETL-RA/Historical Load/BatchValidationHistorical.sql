...
DROP FUNCTION IF EXISTS BatchValidationHistorical();
CREATE FUNCTION BatchValidationHistorical()
RETURNS VOID AS $$
BEGIN
...
from (
    ...
    union select 'DimCustomer' as MessageSource, 'Inactive customers' as MessageText, count(*) from DimCustomer where IsCurrent::INT = 1 and Status = 'Inactive'
