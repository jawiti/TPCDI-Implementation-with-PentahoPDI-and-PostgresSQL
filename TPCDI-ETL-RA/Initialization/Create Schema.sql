DROP FUNCTION IF EXISTS CreateSchema();
CREATE FUNCTION CreateSchema()
RETURNS VOID AS $$
BEGIN

--DROP DATABASE IF EXISTS tpcdiDWRA;
--CREATE DATABASE tpcdiDWRA;


	DROP TABLE IF EXISTS 
	Audit, TradeType, StatusType, customerkeys,customerkeysinact,
	Financial, Industry, FactWatches, FactMarketHistory,
	FactHoldings, FactCashBalances, DImessages, DimTrade,
	DimTime, DimSecurity, DimCompany, factmarkethistoryforexperiment, 
	TaxRate, Prospect, DimDate ,DimBroker, DimCustomer, DimAccount;
...
CREATE TABLE Audit ( DataSet CHAR(20) Not Null,
							BatchID numeric(5),
							Date DATE,
							Attribute CHAR(50),
							Value numeric(15),
...
CREATE TABLE customerkeysinact
(
  sk_customerid integer NOT NULL,
  customerid integer NOT NULL,
  CONSTRAINT customerkeysinact_pkey PRIMARY KEY (sk_customerid)
);

CREATE TABLE factmarkethistoryforexperiment ( 
			id integer,
			dm_date DATE,
			dm_symb CHAR(15),
